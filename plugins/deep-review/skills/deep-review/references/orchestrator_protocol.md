...
4. Decompose the review into research tasks. Run specialist passes for domain-specific investigation, such as the paper's mathematical framework, current literature, methods, empirical design, implementation, or logical structure. Cast a wide net. Each specialist pass should have a distinct, non-overlapping research goal. Run each pass as a separate subagent (Claude's `Agent`, Codex's task/spawn tool, etc.); inline passes don't count.
